...
export class DatatableEditorProvider implements vscode.CustomEditorProvider<DatatableDocument> {
    ...
    private async handleEditInWorking(document: DatatableDocument, panel: vscode.WebviewPanel): Promise<void> {
        const workspaceFolders = vscode.workspace.workspaceFolders;
...
<head>
    ...
    <style>
        * {
        ...
        .badge-ro { background: var(--vscode-badge-background); color: var(--vscode-badge-foreground); padding: 2px 8px; border-radius: 10px; font-size: 11px; font-weight: 600; letter-spacing: 0.5px; }
        .btn-edit-working { padding: 6px 12px; background: var(--vscode-button-background); color: var(--vscode-button-foreground); border: none; border-radius: 3px; cursor: pointer; font-size: 13px; white-space: nowrap; }
        .btn-edit-working:hover { background: var(--vscode-button-hoverBackground); }

        /* Toast */
        .toast { position: fixed; bottom: 20px; left: 50%; transform: translateX(-50%); background: var(--vscode-badge-background); color: var(--vscode-badge-foreground); padding: 8px 20px; border-radius: 6px; font-size: 13px; opacity: 0; transition: opacity 0.3s; pointer-events: none; z-index: 100; }
...
<body>
    ...
    <script>
        ...
        function applyReadOnly(ro) {
            ...
            addRowBtn.disabled = ro;
            deleteRowBtn.disabled = ro || selectedRowIndex < 0;
            duplicateRowBtn.disabled = ro || selectedRowIndex < 0;
        }
        ...
        window.addEventListener('message', event => {
            ...
            switch (message.type) {
                case 'load':
                    ...
                    if (message.readOnly !== undefined) applyReadOnly(message.readOnly);
                    applyFilter();
                    break;
                case 'toast':
        ...
        function updateButtons() {
            ...
            deleteRowBtn.disabled = readOnly || !hasSelection;
            duplicateRowBtn.disabled = readOnly || !hasSelection;
        }
