...
export class PaletteEditorProvider implements vscode.CustomEditorProvider<PaletteDocument> {
    ...
    private async handleEditInWorking(document: PaletteDocument, panel: vscode.WebviewPanel): Promise<void> {
        const workspaceFolders = vscode.workspace.workspaceFolders;
    ...
    private _getCss(): string {
        const lines = [
            ...
            '.badge-ro { background: var(--badge-bg); color: var(--badge-fg); padding: 2px 8px; border-radius: 10px; font-size: 11px; font-weight: 600; letter-spacing: 0.5px; }',
            '.btn-edit-working { background: var(--btn-bg); color: var(--btn-fg); border: none; padding: 4px 12px; cursor: pointer; border-radius: 2px; font-size: 12px; }',
            '.btn-edit-working:hover { background: var(--btn-hover); }',
            '',
            '/* Toast */',
            '.toast { position: fixed; bottom: 20px; left: 50%; transform: translateX(-50%); background: var(--badge-bg); color: var(--badge-fg); padding: 8px 20px; border-radius: 6px; font-size: 13px; opacity: 0; transition: opacity 0.3s; pointer-events: none; z-index: 100; }',
    ...
    private _getScript(): string {
        const lines = [
            ...
            '  btnAdd.disabled = ro;',
            '  btnDuplicate.disabled = ro || selectedIndex < 0;',
            '  btnRemove.disabled = ro || selectedIndex < 0;',
            '  detailPicker.disabled = ro;',
            '  detailHex.disabled = ro;',
            '  detailR.disabled = ro;',
            '  detailG.disabled = ro;',
            '  detailB.disabled = ro;',
            '}',
            ...
            '  if (e.key === "Delete" && selectedIndex >= 0 && !readOnly) {',
            '    btnRemove.click();',
            '  }',
