...
export class DDSEditorProvider implements vscode.CustomEditorProvider<DDSDocument> {
    ...
    private async handleEditInWorking(document: DDSDocument, panel: vscode.WebviewPanel): Promise<void> {
        const workspaceFolders = vscode.workspace.workspaceFolders;
        ...
        if (!relPath) { vscode.window.showWarningMessage('Cannot determine TRE-relative path'); return; }
        ...
        panel.webview.postMessage({ type: 'toast', message: 'Copied to ' + workingDir + '/' + relPath });
        await vscode.commands.executeCommand('vscode.open', vscode.Uri.file(workingPath));
    }
    ...
    async resolveCustomEditor(
        ...
        webviewPanel.webview.onDidReceiveMessage(async e => {
            switch (e.type) {
                case 'ready':
                    this.sendLoad(document, webviewPanel, readOnly);
                    break;
                case 'editInWorking':
    ...
    private sendLoad(document: DDSDocument, panel: vscode.WebviewPanel, readOnly = false): void {
        if (document.error) {
...
/* Read-only badge & button */
.badge-ro { padding: 2px 8px; background: var(--vscode-badge-background); color: var(--vscode-badge-foreground); border-radius: 10px; font-size: 11px; font-weight: 600; letter-spacing: 0.5px; }
.btn-edit-working { padding: 4px 12px; background: var(--vscode-button-background); color: var(--vscode-button-foreground); border: none; border-radius: 3px; cursor: pointer; font-size: 12px; }
.btn-edit-working:hover { background: var(--vscode-button-hoverBackground); }

/* Toast */
.toast { position: fixed; bottom: 20px; left: 50%; transform: translateX(-50%); background: var(--vscode-badge-background); color: var(--vscode-badge-foreground); padding: 8px 20px; border-radius: 6px; font-size: 13px; opacity: 0; transition: opacity 0.3s; pointer-events: none; z-index: 200; }
...
<!-- Info Bar -->
<div class="info-bar">
    <span class="badge-ro" id="badgeRO" style="display:none">READ-ONLY</span>
    <button class="btn-edit-working" id="btnEditWorking" style="display:none">Edit in Working</button>
...
const badgeRO = document.getElementById('badgeRO');
const btnEditWorking = document.getElementById('btnEditWorking');
...
function applyReadOnly(ro) {
    readOnly = ro;
    badgeRO.style.display = ro ? '' : 'none';
    btnEditWorking.style.display = ro ? '' : 'none';
    // Disable editing tools
    document.getElementById('btnCrop').disabled = ro;
    document.getElementById('btnResize').disabled = ro;
    document.getElementById('btnFlipH').disabled = ro;
    document.getElementById('btnFlipV').disabled = ro;
    document.getElementById('btnRotCW').disabled = ro;
    document.getElementById('btnRotCCW').disabled = ro;
    document.getElementById('btnRot180').disabled = ro;
    document.getElementById('btnAdjApply').disabled = ro;
    // Disable sliders
    sliders.brightness.disabled = ro;
    sliders.contrast.disabled = ro;
    sliders.saturation.disabled = ro;
    sliders.hue.disabled = ro;
}

btnEditWorking.addEventListener('click', function() {
    vscode.postMessage({ type: 'editInWorking' });
});
...
window.addEventListener('message', function(e) {
    ...
    switch (msg.type) {
        case 'load': {
            ...
            if (msg.readOnly !== undefined) applyReadOnly(msg.readOnly);
            resetSliders();
            fitToView();
            break;
        }
