...
export class STFEditorProvider implements vscode.CustomEditorProvider<STFDocument> {
    ...
    private async handleEditInWorking(document: STFDocument, panel: vscode.WebviewPanel): Promise<void> {
        const workspaceFolders = vscode.workspace.workspaceFolders;
...
<head>
    ...
    <style>
        * {
        ...
        .badge.ro { background: #c53535; color: #fff; padding: 2px 8px; border-radius: 10px; font-size: 11px; }
        .tbtn { background: var(--vscode-button-background); color: var(--vscode-button-foreground); border: none; padding: 4px 12px; border-radius: 3px; cursor: pointer; font-size: 12px; }
        .tbtn:hover { background: var(--vscode-button-hoverBackground); }
        .tbtn.secondary { background: var(--vscode-button-secondaryBackground); color: var(--vscode-button-secondaryForeground); }
        .tbtn.secondary:hover { background: var(--vscode-button-secondaryHoverBackground); }
        .toast { position: fixed; bottom: 16px; right: 16px; background: var(--vscode-notificationsBackground, #333); color: var(--vscode-notificationsForeground, #ccc); padding: 8px 16px; border-radius: 4px; font-size: 12px; opacity: 0; transition: opacity 0.3s; z-index: 100; }
...
<body>
    <div class="toolbar">
        ...
        <button class="tbtn secondary" id="btnEditWorking" style="display:none">Edit in Working</button>
    ...
    <script>
        ...
        function applyReadOnly() {
            if (readOnly) {
                document.getElementById('badgeRO').style.display = '';
                document.getElementById('btnEditWorking').style.display = '';
                addBtn.disabled = true;
                addBtn.style.opacity = '0.4';
                deleteBtn.style.opacity = '0.4';
            } else {
                document.getElementById('badgeRO').style.display = 'none';
                document.getElementById('btnEditWorking').style.display = 'none';
            }
        }

        document.getElementById('btnEditWorking').addEventListener('click', function() {
