/**
 * DDS Texture Codec - Pure TypeScript DXT1/DXT5 decode + encode + mipmap generation.
 */
...
const DDSD_FLAGS = 0x000A1007;
const DDPF_FOURCC = 0x00000004;
const DDSCAPS = 0x00401008;
...
function encodeDXT5Block(block: Uint8Array): Uint8Array {
    ...
    let bits = 0;
    for (let i = 0; i < 8; i++) { bits |= (alphaIndices[i] & 0x7) << (i * 3); }
    out[2] = bits & 0xFF;
    out[3] = (bits >> 8) & 0xFF;
    out[4] = (bits >> 16) & 0xFF;

    bits = 0;
    for (let i = 0; i < 8; i++) { bits |= (alphaIndices[8 + i] & 0x7) << (i * 3); }
    out[5] = bits & 0xFF;
    out[6] = (bits >> 8) & 0xFF;
    out[7] = (bits >> 16) & 0xFF;
...
function writeHeader(view: DataView, width: number, height: number, fourCC: string, mipCount: number): void {
    ...
    view.setUint32(8, DDSD_FLAGS, true);
    ...
    view.setUint32(108, DDSCAPS, true);
}
...
/**
 * Get DDS info without full decode (header only).
 */
export function getDDSInfo(data: Uint8Array): DDSInfo | null {
