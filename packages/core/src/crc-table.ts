/**
 * CRC String Table (CSTB) Parser/Serializer
 *
 * Format Structure:
 *   FORM CSTB
 *   └── FORM 0000
...
export function parseCRCTable(data: Uint8Array): CRCTable {
    ...
    const outerSize = view.getUint32(pos, false);
    pos += 4;
...
export function serializeCRCTable(table: CRCTable): Uint8Array {
    ...
    pos += 4;
    view.setUint32(pos, outerContentSize, false);
    pos += 4;
    ...
    writeTag(result, pos, 'FORM');
    pos += 4;
    view.setUint32(pos, innerContentSize + 4, false);
    pos += 4;
    ...
    pos += 4;
    view.setUint32(pos, entryCount, true);
    pos += 4;
...
/**
 * Add entries to a CRC table (computes CRCs automatically)
 */
export function addCRCEntries(table: CRCTable, paths: string[]): CRCEntry[] {
    const added: CRCEntry[] = [];
    for (const path of paths) {
        const crc = calculateCRC(path);
        if (!table.entries.some(e => e.crc === crc)) {
            const entry = { crc, path };
...
/**
 * Check if a path exists in the CRC table
 */
export function hasCRCEntry(table: CRCTable, path: string): boolean {
    const crc = calculateCRC(path);
    return table.entries.some(e => e.crc === crc);
}
