...
export class CRCEditorProvider implements vscode.CustomEditorProvider<CRCEditorDocument> {
    ...
    private async handleEditInWorking(document: CRCEditorDocument, panel: vscode.WebviewPanel): Promise<void> {
        const workspaceFolders = vscode.workspace.workspaceFolders;
...
<head>
    ...
    <style>
        * {
        ...
        .btn-edit-working { padding: 4px 12px; background: var(--vscode-button-secondaryBackground); color: var(--vscode-button-secondaryForeground); border: none; border-radius: 3px; cursor: pointer; font-size: 12px; }
        .btn-edit-working:hover { background: var(--vscode-button-secondaryHoverBackground); }
        .toast { position: fixed; bottom: 16px; right: 16px; background: var(--vscode-editorWidget-background, #333); color: var(--vscode-foreground); padding: 8px 16px; border-radius: 4px; font-size: 12px; opacity: 0; transition: opacity 0.3s; z-index: 101; }
