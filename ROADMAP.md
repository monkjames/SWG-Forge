...
### 1c. Object from Existing Appearance ✅ COMPLETE
...
## 2. Creature Duplicator ✅ COMPLETE
...
Tools built since the roadmap was written:

- **ACM Editor** — Browse and edit Asset Customization Manager entries (palette/color variables per appearance CRC)
- **Appearance Chain Editor** — Edit full SWG appearance chains (APT→LOD→MSH→SHT) with inline IFF trees and bulk Save All
- **Art Workshop** — Generate in-game art objects (paintings, rugs, banners, tapestries) from DDS textures
- **BDB Viewer** — Read-only viewer for SWGEmu Berkeley DB databases with field decoding
- **Cockpit Editor** — Visual editor for CPIT IFF files (camera offsets, zoom, hyperspace)
- **Crafting Component Wizard** — Create crafting component loot items (tissues, hides, venoms) with TRE + Lua
- **Datatable Editor** — Spreadsheet-style editor for SWG Datatable IFF files
- **DDS Editor** — View and edit DDS textures (DXT1/DXT5)
- **FLR Viewer** — Visual floor mesh viewer for SWG building cells
- **ILF Viewer** — 2D interior layout viewer for SWG ILF files
- **Mount Wizard** — Automate making creatures and speeders into tameable mounts
- **Palette Editor** — Visual editor for RIFF PAL palette files
- **SFP Viewer** — Visual footprint viewer for SWG structure placement grids
- **TRE Viewer** — Browse TRE archive file contents
- **TRE Builder** — Package working files back into TRE archives
- **Walkway Builder** — Generate complete walkway tile variants (LAY, SFP, IFF, Lua, CRC, STF)
